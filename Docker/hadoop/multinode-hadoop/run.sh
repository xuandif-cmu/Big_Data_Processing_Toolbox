...
c=1 
while [ $c -le $slaveCount ]; do
    tmpName="slave$c"
    #run base-hadoop:1.0 image  as slave container
    docker run -itd  --network="hadoopNetwork"  --ip "172.25.0.10$c" --name $tmpName --hostname $tmpName  base-hadoop-multinode:1.0
    c=$(( c + 1 ))
done
